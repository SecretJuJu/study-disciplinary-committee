...
| `/심사`                        | 모든 구성원           | 공개 접수 메시지·스레드를 만든 뒤 버튼으로 현재 스레드 내용을 심사 |
...
API Gateway ──► interactions Lambda
                    ...
                    └─ /심사 ──► draft + prepare SQS ──► type 4 public anchor
                                                     │
                                                     ▼
                                               judge Lambda
...
| 함수         | 환경변수 이름                                              | 역할 권한                                                                               |
| ------------ | ---------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| interactions | `TABLE_NAME`, `JUDGE_QUEUE_URL`, `DISCORD_PUBLIC_KEY`      | DynamoDB Get/Put/Update/Transact, judge queue SendMessage, logs                         |
| judge        | `TABLE_NAME`, `APP_SECRET_ARN`, `DISCORD_DEBUG_CHANNEL_ID` | DynamoDB Get/Put/Update/Transact, judge queue poll/delete/attributes, secret read, logs |
...
- `master` push의 Deploy workflow는 GitHub OIDC로 AWS 역할을 획득하고 Pulumi 적용, Discord endpoint 갱신, 네 command 동기화, Message Content Intent와 thread 생성·읽기·쓰기 권한, 테스트 메시지를 검증한다.
