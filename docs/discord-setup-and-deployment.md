...
- 최근 thread 메시지를 최대 500개까지만 제한적으로 조회하고, 그중 소유자의 최신 일반 텍스트 최대 100개·20,000자만 심사한다. 초과하면 앞부분과 뒷부분을 보존하고 중간 생략 표식을 넣는다. attachment, system/bot/다른 사용자 메시지는 무시한다.
- 판결 anchor에는 최종 결론과 남은 항소 횟수가 표시된다. 이의가 있으면 직전 판결 뒤에 본인 반박을 새 메시지로 작성하고 `📣 항소`를 누른다. 최대 2회이며, 반박이 없거나 OpenAI 크레딧이 없으면 횟수를 차감하지 않는다.
- 항소는 최초 제출과 새 반박 전체를 다시 검토한다. 구체적인 이해·작업 과정·시행착오는 산출물이 없어도 보완 증거가 될 수 있고, 직전 판결의 핵심 부족 사유를 해소하면 결과가 바뀔 수 있다.
- 항소 기간에 다른 구성원이 thread에 남긴 일반 텍스트도 익명화된 보증·참고 진술로 검토된다. 제출 내용과 구체적으로 일치하는 진술은 신뢰도를 높일 수 있지만, 막연한 보증·다수 의견·학습 시간만으로 판정이 바뀌지는 않는다.
...
7. 판결 뒤 본인 반박과 다른 구성원의 보증을 새 메시지로 작성하고 `📣 항소`를 눌러 남은 횟수가 1회 줄고 판결이 갱신되는지 확인한다.
8. 같은 사용자의 `/내기록`에서 전체 심사 횟수는 그대로이고 현재 판정별 횟수·징계 점수가 항소 결과에 맞게 교정되는지 확인한다.
9. SQS judge queue와 DLQ backlog가 0인지 확인한다.

6~7번은 실제 OpenAI 과금이 발생하는 수동 검증이다. 자동 배포 smoke는 OpenAI 심사를 호출하지 않는다.
