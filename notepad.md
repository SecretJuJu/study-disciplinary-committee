...
[2026-09-01 09:28] - 항소 재심 기준

### DECISIONS

- 초기 심사 기준은 유지하고 항소 재심만 중간 수준으로 완화한다. 항소는 최초 제출과 새 자료 전체를 선입견 없이 다시 평가한다.
- 본인 말로 설명한 이해, 구체적 작업 과정, 시행착오, 판단 근거는 코드·문서 산출물이 없어도 보완 증거가 될 수 있다. 산출물 부재만으로 항소를 기각하지 않는다.
- 구체적이고 제출 내용과 독립적으로 일치하는 참여자 진술은 신뢰도를 높일 수 있지만, 막연한 보증·다수 의견·학습 시간만으로는 판정을 바꾸지 않는다.
- 새 자료가 직전 판결의 핵심 부족 사유를 해소하면 결과를 변경한다. 더 불리한 변경은 Structured Output의 사유가 명백한 모순 또는 조작이고 경계 검증을 통과한 경우에만 반영한다.

### LEARNINGS

- 2026-09-01 production 항소 결과의 outcome 필드만 읽기 전용으로 확인했을 때 7건 중 1건은 `insufficient → meaningful`, 1건은 `meaningful → insufficient`, 5건은 유지되어 기존에도 변경 가능하지만 전반적으로 보수적이었다. 제출 원문이나 사용자 식별 정보는 조회하지 않았다.
- 항소에는 최초 제출 최대 10,000자와 직전 판결 뒤의 제출자 반박 최대 5,000자·익명화한 참여자 진술 최대 3,000자가 모두 전달된다.
